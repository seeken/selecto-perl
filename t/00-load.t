use 5.034;
use strict;
use warnings;
use Test::More;

use_ok('Selecto');
use_ok('Selecto::Action');
use_ok('Selecto::Action::Capability');
use_ok('Selecto::Action::Plan');
use_ok('Selecto::Action::Planner');
use_ok('Selecto::Analytics::UnitRegistry');
use_ok('Selecto::Analytics::TransformRegistry');
use_ok('Selecto::Analytics::Pipeline');
use_ok('Selecto::API');
use_ok('Selecto::API::EngineHandler');
use_ok('Selecto::API::ResultFormatter');
use_ok('Selecto::DateShortcut');
use_ok('Selecto::Adapter');
use_ok('Selecto::Adapter::Registry');
use_ok('Selecto::CoDomain');
use_ok('Selecto::Domain');
use_ok('Selecto::Domain::DSL');
use_ok('Selecto::Domain::Overlay');
use_ok('Selecto::Domain::Ref');
use_ok('Selecto::Domain::Registry');
use_ok('Selecto::DuckDB');
use_ok('Selecto::Engine');
use_ok('Selecto::Error');
use_ok('Selecto::Expression');
use_ok('Selecto::FieldPolicy');
use_ok('Selecto::Identifier');
use_ok('Selecto::MSSQL');
use_ok('Selecto::PostgreSQL');
use_ok('Selecto::Query');
use_ok('Selecto::SQL');
use_ok('Selecto::SQLite');
use_ok('Selecto::Statement');
use_ok('Selecto::Stream');
use_ok('Selecto::Write');
use_ok('Selecto::Write::Expression');

done_testing;
